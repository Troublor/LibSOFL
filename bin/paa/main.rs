...
async fn collect_contract_accounts() {
    ...
    let txs = etherscan.get_transactions(&addr, Some(TxListParams {
    ...
    let user_operation_event_topic0: Hash = ToPrimitive::cvt("0x49628fd1471006c1482da88028e9ce4dbb080b815c9b0344d39e5a8e6ec1419f");
    ...
    for tx_chunk in txs.chunks(16) {
        for tx in tx_chunk {
            let tx_hash = tx.hash.value().map(|h| Hash::from_slice(h.as_bytes())).expect("failed to get tx hash");
            thread::sleep(std::time::Duration::from_millis(50));
            let receipt = eth_provider.get_transaction_receipt(tx_hash).await.expect("failed to get receipt").expect("receipt is unavailable");
            for log in receipt.logs {
                if log.topics[0].to_fixed_bytes() != user_operation_event_topic0.to_fixed_bytes() {
                    continue;
                }
                let sender = &log.topics[2].to_fixed_bytes()[12..32];
                let sender = revm_primitives::Address::from_slice(sender);
                if sender != revm_primitives::Address::zero() && !senders.contains(&sender) {
                        senders.insert(sender);
                        let sender_hex = format!("0x{}\n", hex::encode(sender.as_bytes()));
                ...
                if paymaster != revm_primitives::Address::zero() && !paymasters.contains(&paymaster) {
                    paymasters.insert(paymaster);
                    let paymaster_hex = format!("0x{}\n", &*hex::encode(paymaster.as_bytes()));
